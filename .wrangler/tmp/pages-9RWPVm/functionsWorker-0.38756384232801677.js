...
var AuthDatabase = class {
   ...
   */
  async updateSuccessfulLogin(userId, ipAddress) {
    const stmt = this.db.prepare(`
      UPDATE users 
      SET 
   ...
   */
  async updateFailedLogin(userId) {
    const stmt = this.db.prepare(`
      UPDATE users 
      SET 
    ...
    await stmt.bind(
      ...
      AUTH_CONFIG.ACCOUNT_LOCK_DURATION / 1e3,
      // Convert to seconds
      userId
    ).run();
  }
   ...
   */
  async revokeUserSessions(userId, reason = "new_login") {
    const stmt = this.db.prepare(`
      UPDATE sessions 
      SET 
        is_revoked = 1,
...
var onRequestPost2 = /* @__PURE__ */ __name(async (context) => {
  ...
  try {
    ...
    const securityResult = await security.applySecurityChecks(request, {
      ...
      requireAuth: false,
      // Login endpoint doesn't require existing auth
      allowedMethods: ["POST"],
      endpoint: "login"
    });
    if (!securityResult.allowed) {
    ...
    const authUser = {
      ...
      lastLoginAt: user.last_login_at ? new Date(user.last_login_at).getTime() : void 0
    };
    ...
    await Promise.all([
      ...
      db.revokeUserSessions(user.id, "new_login"),
...
// api/auth/logout.ts
var SessionDatabase = class {
  constructor(db) {
    this.db = db;
  }
  static {
    __name(this, "SessionDatabase");
  }
  /**
   * Revoke specific session by access token
   */
  async revokeSessionByToken(accessToken, reason = "user_logout") {
    const stmt = this.db.prepare(`
      UPDATE sessions 
      SET 
        is_revoked = 1,
   ...
   */
  async revokeAllUserSessions(userId, reason = "logout_all_devices") {
    const stmt = this.db.prepare(`
      UPDATE sessions 
      SET 
        is_revoked = 1,
   ...
   */
  async getSessionDetails(accessToken) {
    const stmt = this.db.prepare(`
      SELECT 
...
var LogoutValidator = class {
  static {
    __name(this, "LogoutValidator");
  }
  /**
   * Validate logout request payload
   */
  static validateLogoutRequest(data) {
    const errors = [];
    if (data !== null && data !== void 0) {
      if (typeof data !== "object" || Array.isArray(data)) {
...
var onRequestPost3 = /* @__PURE__ */ __name(async (context) => {
  ...
  try {
    ...
    const securityResult = await security.applySecurityChecks(request, {
      rateLimitConfig: RATE_LIMIT_CONFIGS.API,
      // Use general API rate limiting
      requireAuth: true,
      // Logout requires valid authentication
      allowedMethods: ["POST"],
      endpoint: "logout"
    });
    if (!securityResult.allowed) {
    ...
    let logoutRequest = { logoutAllDevices: false };
    if (request.headers.get("Content-Type")?.includes("application/json")) {
      const validationResult = await SecurityHelpers.validateRequest(
        request,
    ...
    const responseData = {
      success: true,
      message: logoutRequest.logoutAllDevices ? "Successfully logged out from all devices" : "Successfully logged out",
      data: {
        logoutType,
...
// api/auth/refresh.ts
var RefreshDatabase = class {
  constructor(db) {
    this.db = db;
  }
  static {
    __name(this, "RefreshDatabase");
  }
  /**
   ...
   */
  async updateSession(sessionId, newTokens, ipAddress) {
    const stmt = this.db.prepare(`
      UPDATE sessions 
      SET 
        access_token = ?,
...
function validateRefreshTokenRequest(data) {
  const errors = [];
  if (!data || typeof data !== "object") {
    return { isValid: false, errors: ["Invalid request data"] };
  }
  ...
  } else if (data.refreshToken.length > 2048) {
...
__name(validateRefreshTokenRequest, "validateRefreshTokenRequest");
var onRequestPost4 = /* @__PURE__ */ __name(async (context) => {
  ...
  try {
    ...
    const securityResult = await security.applySecurityChecks(request, {
      ...
      requireAuth: false,
      // Refresh doesn't require valid access token
      allowedMethods: ["POST"],
      endpoint: "refresh"
    });
    if (!securityResult.allowed) {
    ...
    const newTokens = await JWTManager.generateTokens(authUser, env.JWT_SECRET);
...
// ../constants.js
var MIN_RAW_REQUEST_LENGTH = 10;
var translations = {
  fr: {
    app: {
      title: "Prompt Builder Teachinspire",
      subtitle: "Transformez vos id\xE9es en prompts structur\xE9s efficaces"
    },
    input: {
      placeholder: "D\xE9crivez ce que vous voulez faire avec l'IA...",
      button: "Analyser ma demande",
      charCount: "caract\xE8res",
      minCharWarning: `Minimum ${MIN_RAW_REQUEST_LENGTH} caract\xE8res requis`
    },
    analysis: {
      title: "Analyse de votre demande",
      domain: "Domaine d\xE9tect\xE9",
      complexity: "Complexit\xE9 estim\xE9e",
      recommendation: "Approche recommand\xE9e",
      simple: "Simple",
      complex: "Complexe"
    },
    approach: {
      ...
      mvp: {
        title: "MVP",
        subtitle: "(Recommand\xE9 pour t\xE2ches simples)",
        description: "Structure simple System-User-Exemple"
      },
      agentique: {
        title: "AGENTIQUE",
        subtitle: "(Pour t\xE2ches complexes)",
        description: "Avec auto-\xE9valuation et it\xE9ration"
      }
    },
    variables: {
      ...
      expertRole: "R\xF4le de l'expert",
      expertRoleDescription: "Pr\xE9ciser le r\xF4le que vous voulez donner \xE0 l'IA pour cette t\xE2che - sa fonction.",
      expertRolePlaceholder: "Ex: Concepteur p\xE9dagogique",
      mission: "Mission principale",
      missionDescription: "Pr\xE9ciser les t\xE2ches que vous voulez que l'IA ex\xE9cute - sa mission.",
      missionPlaceholder: "Ex: cr\xE9er des cours engageants",
      constraints: "Contraintes (une par ligne)",
      next: "Suivant",
      back: "Retour",
      constraintsPlaceholder: "Ex:\nDur\xE9e: 50 minutes\nNiveau: B1\nGroupe: 12 \xE9l\xE8ves"
    },
    generation: {
      generating: "G\xE9n\xE9ration en cours...",
      title: "Votre prompt structur\xE9",
      error: "Erreur de g\xE9n\xE9ration. Veuillez r\xE9essayer."
    },
    actions: {
      copy: "Copier",
      save: "Sauvegarder",
      export: "Exporter",
      generate: "G\xE9n\xE9rer le prompt",
      newPrompt: "Nouveau prompt",
      viewLibrary: "Mes prompts",
      copiedSuccess: "Copi\xE9!",
      copyError: "Erreur de copie",
      savedSuccess: "Sauvegard\xE9!",
      usePrompt: "Utiliser",
      delete: "Supprimer"
    },
    library: {
      title: "Biblioth\xE8que de prompts",
      empty: "Aucun prompt sauvegard\xE9",
      close: "Fermer"
    },
    domains: {
    ...
    notifications: {
      copied: "Prompt copi\xE9 dans le presse-papiers!",
      copyFailed: "\xC9chec de la copie du prompt.",
      saved: "Prompt sauvegard\xE9 dans la biblioth\xE8que!",
      apiError: "Erreur de l'API Gemini. Veuillez v\xE9rifier votre cl\xE9 API et r\xE9essayer.",
      deleted: "Prompt supprim\xE9 !",
      saveError: "\xC9chec de la sauvegarde du prompt.",
      deleteError: "\xC9chec de la suppression du prompt."
    },
    auth: {
      loading: "Chargement...",
      login: {
        title: "Connexion",
        subtitle: "Connectez-vous \xE0 votre compte Teachinspire",
        signIn: "Se connecter",
        signingIn: "Connexion...",
        noAccount: "Pas encore de compte ?",
        signUpLink: "Cr\xE9er un compte"
      },
      register: {
        title: "Cr\xE9er un compte",
        subtitle: "Rejoignez Teachinspire pour sauvegarder vos prompts",
        createAccount: "Cr\xE9er le compte",
        creatingAccount: "Cr\xE9ation...",
        hasAccount: "D\xE9j\xE0 un compte ?",
        signInLink: "Se connecter",
        passwordRequirements: "Exigences du mot de passe :",
        requirements: {
          length: "Au moins 8 caract\xE8res",
          lowercase: "Une lettre minuscule",
          uppercase: "Une lettre majuscule",
          number: "Un chiffre",
          special: "Un caract\xE8re sp\xE9cial"
        }
      },
      fields: {
      ...
      validation: {
        ...
        passwordMinLength: "Minimum 6 caract\xE8res",
        passwordMinLength8: "Minimum 8 caract\xE8res",
        confirmPasswordRequired: "Veuillez confirmer votre mot de passe",
        passwordsDoNotMatch: "Les mots de passe ne correspondent pas"
      },
      errors: {
        loginFailed: "\xC9chec de la connexion. V\xE9rifiez vos identifiants.",
        registrationFailed: "\xC9chec de l'inscription. Veuillez r\xE9essayer.",
        networkError: "Erreur r\xE9seau. V\xE9rifiez votre connexion.",
      ...
      migration: {
        ...
        description: "Nous transf\xE9rons vos prompts sauvegard\xE9s vers votre compte...",
        progress: "Migration en cours: {completed}/{total} prompts",
        success: "Migration r\xE9ussie! {migrated} prompts transf\xE9r\xE9s",
        successWithErrors: "Migration termin\xE9e avec {migrated} prompts transf\xE9r\xE9s et {failed} \xE9checs",
        failed: "\xC9chec de la migration. Veuillez r\xE9essayer.",
        retry: "R\xE9essayer la migration",
        skip: "Ignorer pour l'instant",
        detecting: "D\xE9tection des prompts \xE0 migrer...",
        backup: "Sauvegarde des donn\xE9es en cours...",
        uploading: "Envoi des prompts...",
        finalizing: "Finalisation...",
        noPromptsFound: "Aucun prompt \xE0 migrer trouv\xE9",
        partialSuccess: "Migration partiellement r\xE9ussie",
        errors: {
          networkError: "Erreur r\xE9seau lors de la migration",
          validationError: "Erreur de validation des donn\xE9es",
          serverError: "Erreur serveur lors de la migration",
          backupFailed: "\xC9chec de la sauvegarde des donn\xE9es",
...
var PromptDatabase = class {
  constructor(db) {
    this.db = db;
  }
  static {
    __name(this, "PromptDatabase");
  }
  /**
  ...
  async savePrompt(userId, promptData) {
    ...
    `);
    const title = promptData.rawRequest.substring(0, 97) + (promptData.rawRequest.length > 97 ? "..." : "");
    await stmt.bind(
      promptId,
      userId,
      title,
...
var onRequestPost6 = /* @__PURE__ */ __name(async (context) => {
  ...
  try {
    ...
    const clientIP = AuthUtils.getClientIP(request);
    const userAgent = AuthUtils.getUserAgent(request);
    const authenticatedUserId = securityResult.userId;
    ...
    const tApp = translations[params.language] || translations.en;
...
// api/prompts.ts
var PromptsValidator = class {
  static {
    __name(this, "PromptsValidator");
  }
  /**
   * Validate GET query parameters
   */
  static validateGetPromptsQuery(searchParams) {
    const errors = [];
    const filters = {};
    const pagination = {};
   ...
   */
  static validateCreatePromptRequest(data) {
    const errors = [];
    if (!data || typeof data !== "object" || Array.isArray(data)) {
      return { isValid: false, errors: ["Invalid request data structure"] };
    }
    ...
    } else if (data.generatedPrompt.length > 1e4) {
    ...
    const sanitizedData = {
      ...
      language: data.language,
      outputLength: data.outputLength,
      title: data.title ? InputValidator.sanitizeInput(data.title) : void 0,
      expertRole: data.expertRole ? InputValidator.sanitizeInput(data.expertRole) : void 0,
      mission: data.mission ? InputValidator.sanitizeInput(data.mission) : void 0,
      constraints: data.constraints ? InputValidator.sanitizeInput(data.constraints) : void 0,
...
var onRequestOptions5 = /* @__PURE__ */ __name(async (context) => {
  const { request, env } = context;
  try {
    const security = new SecurityMiddleware(env.KV, env.JWT_SECRET);
    const securityResult = await security.applySecurityChecks(request, {
...
// ../.wrangler/tmp/pages-9RWPVm/functionsRoutes-0.8604560045213743.mjs
var routes = [
  {
